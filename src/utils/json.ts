...
export function parseError(e: any, jsonStr: string, t: any): string {
  ...
  if (typeof a === 'object') {
    if (Array.isArray(a) && Array.isArray(b)) {
      ...
      for (let i = 0; i < maxLen; i++) {
        const p = `${path}[${i}]`
        if (i >= a.length) result.push({ path: p, type: 'added', b: JSON.stringify(b[i]) })
        else if (i >= b.length) result.push({ path: p, type: 'removed', a: JSON.stringify(a[i]) })
        else result.push(...diffJSON(a[i], b[i], p))
      }
    } else {
      const allKeys = new Set([...Object.keys(a), ...Object.keys(b)])
      for (const key of allKeys) {
        const p = path ? `${path}.${key}` : key
        if (!(key in a)) result.push({ path: p, type: 'added', b: JSON.stringify(b[key]) })
        else if (!(key in b)) result.push({ path: p, type: 'removed', a: JSON.stringify(a[key]) })
        else result.push(...diffJSON(a[key], b[key], p))
      }
    }
  } else {
