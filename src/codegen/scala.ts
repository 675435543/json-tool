...
function scalaType(val: any): string {
  ...
  if (Array.isArray(val)) {
    ...
    const inners = val.map(v => scalaType(v))
...
export function generateScala(obj: any, rootName: string): string {
  ...
  function process(obj: any, name: string): void {
    ...
    for (const [key, val] of Object.entries(obj)) {
      ...
      if (val !== null && typeof val === 'object' && !Array.isArray(val)) {
        ...
        fields.push({ key: fname, type: scalaType(val) })
      }
    }

    const props = fields.map(f => `  ${f.key}: ${f.type}`)
